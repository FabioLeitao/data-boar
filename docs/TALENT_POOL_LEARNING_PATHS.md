...
| **Next 1–2 quarters** | Scoped PRs: pin actions, Scout follow-ups, or doc runbooks — match `docs/plans/PLANS_TODO.md` maintenance rows when possible.                                                            | **SRE** or **platform** certs (see PORTFOLIO CV-derived list) align with this path; keep **supply-Colleague-Nn** mindset (pinned actions, provenance) as a concrete portfolio story. |
