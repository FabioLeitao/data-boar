...
| **UE (DSA Art. 28 + EUDI Wallet — atestação de idade)** | [compliance-sample-eu_age_verification.yaml](compliance-samples/compliance-sample-eu_age_verification.yaml) | Mapeamento **técnico** de *age assurance* europeu — proteção de menores no DSA Art. 28 + atestação de idade na EUDI Wallet (eIDAS 2.0); **não** realiza verificação de idade nem aconselhamento jurídico. |
