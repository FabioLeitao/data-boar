...
| **EUA (H.R. 8250 — Parents Decide Act, pré-promulgação)** | [compliance-sample-us_hr8250_parents_decide.yaml](compliance-samples/compliance-sample-us_hr8250_parents_decide.yaml) | **H.R. 8250** (119º Congresso, *introduzido — não promulgado*) e *App Store Accountability Act* (companion no Senado) — **léxico técnico** para registros de **consentimento parental**, **age signals** no nível do sistema operacional, **age gates** em app stores e vínculo **dispositivo → responsável**. **Não** verifica idade, **não** impõe consentimento, **não** equivale a parecer jurídico. |
