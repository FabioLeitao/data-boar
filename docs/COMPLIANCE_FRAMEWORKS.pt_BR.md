...
**Amostras de conformidade (um perfil por regulamento):** Amostras de config para **UK GDPR**, **PIPEDA**, **POPIA**, **APPI** e **PCI-DSS** estão em [compliance-samples/](compliance-samples/). Cada amostra é um único arquivo YAML com padrões regex, termos ML e recommendation overrides para esse framework. Use-as assim:
...
**Amostra disponível:** [compliance-sample-uk_gdpr.yaml](compliance-samples/compliance-sample-uk_gdpr.yaml) – UK GDPR (Reino Unido pós-Brexit, alinhado à ICO). Mais amostras (PIPEDA, POPIA, APPI, PCI-DSS) serão adicionadas no mesmo formato; veja o [README de compliance-samples](compliance-samples/README.pt_BR.md).
