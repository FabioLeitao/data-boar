...
- **SonarQube Server:** Add secrets `SONAR_TOKEN` and `SONAR_HOST_URL` (e.g. `https://sonarqube.your-company.com`). For a **self-hosted** instance on a home lab (Docker, tokens, reverse proxy, and how to make GitHub Actions reach it), see **[SONARQUBE_HOME_LAB.md](ops/SONARQUBE_HOME_LAB.md)** ([pt-BR](ops/SONARQUBE_HOME_LAB.pt_BR.md)).
