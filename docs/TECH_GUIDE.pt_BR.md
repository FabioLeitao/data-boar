...
- **Docker Hub:** [hub.docker.com/r/fabioleitao/data_boar](https://hub.docker.com/r/fabioleitao/data_boar) — `fabioleitao/data_boar:latest` e `fabioleitao/data_boar:1.7.0` (estável). O **`main`** pode refletir um semver de **pré-release** (ex.: **1.7.1-beta** no `pyproject.toml`) até a próxima tag estável; ver [CHANGELOG.md](../CHANGELOG.md) e [releases/1.7.1-beta.md](releases/1.7.1-beta.md).
