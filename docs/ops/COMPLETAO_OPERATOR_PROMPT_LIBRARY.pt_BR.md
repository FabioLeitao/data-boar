...
| **`tier:closure-min`** | Fechamento minimo pos-sessao (mais que smoke, menos que "tudao") | Completar os 3 pendentes principais: (1) repetir CLI soup em pelo menos mais 1 host alem de `latitude`; (2) validar API + dashboard com `GET /health` + `GET /`; (3) consolidar nota privada + lições aprendidas da rodada. |
