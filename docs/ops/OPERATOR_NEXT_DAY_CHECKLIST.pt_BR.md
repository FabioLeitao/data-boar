...
| M3 | **LAB-OP — `<lab-host-2>` / `LAB-NODE-04`:** resolver `git status` no script `homelab-host-report.sh` **ou** correr `lab-op-sync-and-collect.ps1 -SkipGitPull` só para relatório         | Log novo em `docs/private/homelab/reports/` **ou** pull limpo         |
| M4 | **ThinkPad LAB-NODE-01 + LMDE 7:** continuar [LMDE7_LAB-NODE-01_DEVELOPER_SETUP.pt_BR.md](LMDE7_LAB-NODE-01_DEVELOPER_SETUP.pt_BR.md) (mapa §0–§8) até checklist §8 verde **ou** marcar onde paraste | Seção atual documentada numa linha em nota privada                    |

Nota: se não tiveres LAB-NODE-01 à mão, troca M4 por mais uma linha de Band A ou M3.
...
| T2 | **Corporate-Entity-C (opcional):** se quiseres fechar o loop com revisores — enviar e-mail com caminhos de [Corporate-Entity-C_IN_REPO_BASELINE.md](Corporate-Entity-C_IN_REPO_BASELINE.md) e referência a `Corporate-Entity-C_ANALISE_2026-03-18.md` | Mensagem enviada **ou** rascunho guardado + data para enviar                         |
...
| N2 | Se o **Task Scheduler SNMP** estiver activo — abrir última linha do `snmp_LAB-ROUTER-01_probe_*.log` em `reports/`                                                              | Olhar “OK”/erros; nada a fazer se estiver verde |
...
- SSH aos hosts, `sudo` no LAB-NODE-01, abrir browser no GitHub com a tua conta.
- Enviar e-mail à Corporate-Entity-C — só pode preparar texto.
