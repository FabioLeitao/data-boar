...
| M3 | **LAB-OP — `mini-bt` / `pi3b`:** resolver `git status` no script `homelab-host-report.sh` **ou** correr `lab-op-sync-and-collect.ps1 -SkipGitPull` só para relatório         | Log novo em `docs/private/homelab/reports/` **ou** pull limpo         |
