...
Tracked examples and scripts use the SSH alias **`lab-node-02`** for the Linux lab server (Zorin; Docker, reports). Configure **`Host lab-node-02`** in your dev PC’s **`~/.ssh/config`** so it matches **DNS/mDNS on the LAN** (and **ed25519** keys as pre-authorized on the host). See **`docs/private.example/homelab/README.md`**.
