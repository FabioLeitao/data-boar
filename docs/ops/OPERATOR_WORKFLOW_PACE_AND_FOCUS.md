...
3. **Corporate-Entity-C loop:** Reply to Corporate-Entity-C with **evidence** (paths in `Corporate-Entity-C_IN_REPO_BASELINE.md`); pick **one** next row from the evolution table (e.g. product naming or secure-default API decision) as a **scoped** `feature` or `backlog` slice.
