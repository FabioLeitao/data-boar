...
There is **no** `bitwarden-cli` package in Void’s default `xbps` repos. Use **Node + npm** from Void, then the official npm package (same idea as [Ansible role `lab-node-01_bitwarden_cli`](../../ops/automation/ansible/roles/lab-node-01_bitwarden_cli/tasks/main.yml) on Debian):
