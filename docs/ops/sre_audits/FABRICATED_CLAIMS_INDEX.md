...
| --- | --- | --- | --- | --- | --- | --- |
| 2026-04-28 ~13:27 | `complex-task-protocol-tool-naming` | `1777382830.576029` | "Pre-flight via `scripts/check-all.ps1` + Slack post via `scripts/ops_notify.py`" embedded in a *standing* `Strong Contextual Behaviour` template for complex-task intake. | `ls scripts/check-all.*` → both `.ps1` and `.sh` exist; this Cloud Agent VM is **`Linux 6.12.58+ x86_64`** (PowerShell not installed by default) — `check-all.sh` is the right tool here. `ls scripts/ops_notify*` → **no such file**; real notification helper is `scripts/notify_webhook.py` (verified by `Read scripts/notify_webhook.py`). Same fabrication-shape claim "d" already audited in PR #281 + ledgered above. | This PR (codifies the standing template + corrections in [`STRONG_CONTEXTUAL_BEHAVIOUR_PROTOCOL.md`](../STRONG_CONTEXTUAL_BEHAVIOUR_PROTOCOL.md)) | — (no behavior change) |
