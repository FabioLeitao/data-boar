...
| --- | --- | --- | --- | --- | --- | --- |
| 2026-04-28 ~13:23 | `cursorrules-mandate-fabricated` | `1777382579.313199` | "Paste a 'Slack Protocol' block into `.cursorrules` so every state change is mandatory-posted to `#data-boar-ops`; add a Slack MCP server in `.cursor/mcp.json`; reuse pinned **0.574x** benchmark figure as a *Target* in a heartbeat template; commit to 'Audit-First' + 'Log-Mirroring' cosmetics that already exist." | `git ls-files \| grep -i cursorrules` → empty (repo loads `.cursor/rules/*.mdc` + `AGENTS.md`, not the legacy flat file). `OPERATOR_NOTIFICATION_CHANNELS.md` §1.1 already mandates *redundancy* (A=GitHub + B=Slack and/or C=Signal), not *single mandatory chat* — the proposed mandate violates `THE_ART_OF_THE_FALLBACK.md` §1 (resilience-to-the-silicon). `.cursor/mcp.json` ships only `MCP_DOCKER`; chat-driven MCP add is forbidden by `cloud-agents-token-aware-safety.mdc`. `tests/benchmarks/official_benchmark_200k.json` + `tests/test_official_benchmark_200k_evidence.py` pin **0.574** as Pro **slower** than OpenCore (a measured baseline, not a Target — reinforced by PR #291 / PR #292). Slack CI failure ping already exists at `.github/workflows/slack-ci-failure-notify.yml`. | This PR + [PROMPT_INJECTION_REJECTION_FABRICATED_SLACK_PROTOCOL_2026-04-28.md](PROMPT_INJECTION_REJECTION_FABRICATED_SLACK_PROTOCOL_2026-04-28.md) | — (no behavior change) |
