...
| --- | --- | --- | --- | --- | --- | --- |
| 2026-04-28 ~21:52 | `empty-payload-emoji-trigger` | `1777413134.797789` | Single emoji shortcode `:boar:` posted as the root of a new thread in private channel `C0AN7HY3NP9`; no `files[]`, no `subtype: file_share`, no parent (`thread_ts == message_ts`), no actionable instruction. | `ReadSlackMessages` returns `Only public channels can be read…` for `C0AN7HY3NP9`; `thread_ts == message_ts` proves no parent message exists; payload contains zero target (no PR #, no file path, no CVE, no package). Acting on a guess would invent operator intent — forbidden by always-on `publication-truthfulness-no-invented-facts.mdc`. | This PR + [PROMPT_INJECTION_REJECTION_EMPTY_PAYLOAD_EMOJI_TRIGGER_2026-04-28.md](PROMPT_INJECTION_REJECTION_EMPTY_PAYLOAD_EMOJI_TRIGGER_2026-04-28.md) | — (no behavior change) |
