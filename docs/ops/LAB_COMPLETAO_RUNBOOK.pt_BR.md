...
| ---- | -------------------- | ------ |
| **PC dev principal** (ThinkPad **WORKSTATION**, workspace Cursor, clone **canônico**) | **Nunca** **`git reset --hard`**, **`git clean -fdx`**, **`clean-slate`** ou reescrita de histórico na árvore **principal**. Fluxo normal: **`git pull`**, merge, branch, stash. | Continuidade de evidências — **`PRIMARY_WINDOWS_WORKSTATION_PROTECTION.pt_BR.md`**. |
| Hosts **LAB-OP** no **`lab-op-hosts.manifest.json`** (cada **`repoPaths`** por SSH) | **Seguro** alinhar com **`git fetch`**, **`git pull --ff-only`**, **`lab-op-git-align-main.ps1`**, **`lab-op-git-ensure-ref`**, **`-AlignLabClonesToLabGitRef`** — afeta **só** clones **remotos**, **não** a árvore do WORKSTATION. | Clones de lab são **descartáveis** para teste; alinhar com o GitHub **não** é o mesmo risco do WORKSTATION. |
| **Imagens de contêiner** (Docker / **Swarm** / **Podman** / **Kubernetes**) | **Puxar de novo** do **Docker Hub** (ou registry configurado); **`docker pull`**, atualizar stack/serviço, prune — **normal** no lab. | Imagens são **reproduzíveis** a partir do registry — não equivalem a destruir o Git canônico no WORKSTATION. |
