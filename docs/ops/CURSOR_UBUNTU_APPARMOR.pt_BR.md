...
Sintomas: crash ao abrir, **terminal integrado** a falhar, **Git** a falhar, ou **gravar ficheiros** em certas árvores—**e** o log do kernel mostra **AppArmor DENIED** para `cursor`, `cursor-sandbox` ou um Contributor-A (ex.: `node`, `bash`).
...
Uso avançado: **`aa-genprof`** / **`aa-logprof`** (ver guia Ubuntu Server — *Security — AppArmor*) a partir de registos de auditoria. Aplicações **Electron** geram muitos Contributor-As—espere várias iterações.
...
- Contexto homelab (ex. **lab-node-02** / Zorin): **`docs/private/homelab/LAB_SECURITY_POSTURE.md`** §2 (UFW, Lynis, menções AppArmor)—**não** substitui este runbook.
