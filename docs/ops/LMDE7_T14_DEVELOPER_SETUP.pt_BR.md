...
**Se o seu caso é ainda mais “duro” (como você descreveu no WORKSTATION):** *Vantage*, *Lenovo System Update* e o instalador baixado do site **nem chegam a mostrar progresso**, só reiniciam e voltam igual.
...
**Nota importante (para “não esquecermos”):** você mencionou que isso está acontecendo também no **ThinkPad WORKSTATION**. Este checklist é o ponto de partida; quando você trouxer as fotos do WORKSTATION, a gente compara as telas equivalentes (Main / Update Option / Secure Boot / TPM / Boot) e decide se há alguma configuração específica que está impedindo a aplicação do update.
