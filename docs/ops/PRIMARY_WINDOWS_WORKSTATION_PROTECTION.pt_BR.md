...
**L-series** é um **papel**: o **PC Windows principal** de desenvolvimento, onde o clone **canônico** do `data-boar`, evidências locais e a **continuidade do histórico Git** devem ser preservados. **Não** é um hostname para commits ou docs públicos. **Âncora de hardware:** o laptop principal do operador costuma ser um ThinkPad **WORKSTATION** (ou similar); **nunca** confundir scripts de alinhamento **LAB-OP** (`lab-op-git-align-main`, `lab-op-git-ensure-ref` em **Reset**, etc.) com a árvore **canônica** nesta máquina — esses scripts atingem só hosts do **manifesto SSH**, não o workspace do Cursor no WORKSTATION.
