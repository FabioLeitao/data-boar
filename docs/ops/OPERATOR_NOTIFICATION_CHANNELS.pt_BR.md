...
1. **CI / Rust / Semgrep falhou → Slack (fase 1):** workflow **`Slack CI failure notify`** (`.github/workflows/slack-ci-failure-notify.yml`) corre quando **`CI`**, **`Rust CI`**, **`Semgrep`** ou **`SBOM`** termina com **`failure`** (push/PR para `main` ou `master`). Usa o **mesmo** `SLACK_WEBHOOK_URL`. A mensagem usa o **nome do workflow** que falhou (ex.: **Test (Python 3.13)**, **cargo (check, test, clippy)** ou **Semgrep (OSS, Python)**). Sem secret → o job de notify fica **skipped**. *PRs de fork:* falhas podem ainda gerar ping — filtrar mais tarde se fizer barulho demais.
...
| CI / Rust / Semgrep falhou (depois) | Após falha real do **`CI`**, **`Rust CI`**, **`Semgrep`** ou **`SBOM`**                      | Run **Slack CI failure notify**; no Slack, texto **“Data Boar — &lt;nome do workflow&gt; falhou”** com link para o run.                      |
