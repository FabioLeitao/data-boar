...
1. **CI / Rust / Semgrep failure ping (phase 1):** workflow **`Slack CI failure notify`** (`.github/workflows/slack-ci-failure-notify.yml`) runs when **`CI`**, **`Rust CI`**, **`Semgrep`**, or **`SBOM`** finishes with **`failure`** (push/PR to `main` or `master`). Uses the **same** `SLACK_WEBHOOK_URL`. If the secret is unset, the notify job is **skipped**. The Slack line uses the failing workflow’s display name (e.g. **Test (Python 3.13)**, **cargo (check, test, clippy)**, or **Semgrep (OSS, Python)**). *Fork PRs:* failures may still produce a ping—reduce noise later (branch filters) if needed.
...
| CI / Rust / Semgrep failure path | After a real **`CI`**, **`Rust CI`**, **`Semgrep`**, or **`SBOM`** failure (or a test branch)                | **Actions** shows **Slack CI failure notify** run; Slack shows **“Data Boar — &lt;workflow name&gt; falhou”** with a link to the failed run. |
