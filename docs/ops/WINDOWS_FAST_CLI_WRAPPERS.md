...
| ------ | ------- | -------- | ----- |
| **`scripts/repo-grep.ps1`** | **`rg`** (ripgrep), optional **`baregrep.exe`** with **`-PreferBareGrep`** | **`Select-String`** (capped file scan) | Skips unrelated **`grep.exe`** (e.g. FPC toolColleague-Nn) for recursive safety. Default **`-MaxOutputLines`** caps transcript size. **`baregrep.exe`** is also searched under **`%USERPROFILE%\Downloads`** (portable drop; not winget). |
...
- **Ripgrep:** `winget install BurntSushi.ripgrep.MSVC` (or another `rg` distribution) so **`repo-grep.ps1`** hits the fast path. Prefer **`rg`** over random **`grep.exe`** installs (toolColleague-Nn collisions); see script comments.
