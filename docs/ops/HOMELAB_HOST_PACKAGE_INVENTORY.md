...
**WSL2 vs another Linux host:** each distro’s **ext4 VHD** has its **own** **`~/.ollama`** (and **`ollama list`**). Same **semver** (**`ollama --version`**) on **WSL** and **lab-node-02** does **not** imply shared weights — **`ollama pull`** must be run where you want the model to run.
...
**No Docker/Podman on host:** use **`uv sync`** on the metal (with the packages above) or run tests in a **Linux/glibc** environment (e.g. **lab-node-02** or the published **Docker** image) when **musl** builds are too painful.
