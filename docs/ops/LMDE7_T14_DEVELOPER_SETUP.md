...
**Português (Brasil):** **canonical concrete steps** — **[LMDE7_LAB-NODE-01_DEVELOPER_SETUP.pt_BR.md](LMDE7_LAB-NODE-01_DEVELOPER_SETUP.pt_BR.md)** (§0–§8: updates, security, Python/`uv`/pytest, optional Podman/Docker, final checklist; **§9** = related links). **Prerequisite:** LMDE 7 must **already boot** on the LAB-NODE-01; **USB installer / disk partitioning** are **out of scope** here (see Mint LMDE + Lenovo UEFI docs), then start at **§0** in the pt-BR file.
