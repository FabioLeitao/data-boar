...
| `completao-chat-starter.ps1` | Print minimal Cursor chat lines (**`completao`** + **`tier:…`**) + suggested command (includes **`release-master-v1-7-3`**) | **`COMPLETAO_OPERATOR_PROMPT_LIBRARY.md`**, **`COMPLETAO_MESTRE_RELEASE_CHECKLIST_PROMPT.md`**, **`LAB_COMPLETAO_FRESH_AGENT_BRIEF.md`** (full blocks A–E when prose changes) |
