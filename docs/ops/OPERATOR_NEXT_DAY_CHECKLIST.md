...
| M3 | **LAB-OP — `mini-bt` / `pi3b`:** fix `git status` on `homelab-host-report.sh` **or** run `lab-op-sync-and-collect.ps1 -SkipGitPull`                               | New report under `docs/private/homelab/reports/` **or** clean pull      |
