...
Since the prior WRB delta snapshot (2026-03-31), we advanced the working tree toward 1.6.8 alignment and tightened operator-facing flows: session keyword / LAB-OP SSH naming consistency (canonical host alias `lab-node-02` in examples), operator session shorthands index, token-aware docs cross-links, and Markdown table hygiene in session-mode rules. Homelab automation scripts and private-example manifests were updated to match the same SSH alias. A verified public bundle for optional external LLM triage (Gemini) is generated with `export_public_gemini_bundle.py --verify` (not a substitute for git/CI/pytest).
