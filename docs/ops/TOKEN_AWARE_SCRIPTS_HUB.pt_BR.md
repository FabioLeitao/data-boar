...
| `completao-chat-starter.ps1` | Imprime linhas mínimas para o chat (**`completao`** + **`tier:…`**) + comando sugerido (inclui **`release-master-v1-7-3`**) | **`COMPLETAO_OPERATOR_PROMPT_LIBRARY.pt_BR.md`**, **`COMPLETAO_MESTRE_RELEASE_CHECKLIST_PROMPT.pt_BR.md`**, **`LAB_COMPLETAO_FRESH_AGENT_BRIEF.pt_BR.md`** (blocos A–E completos quando a prosa muda) |
