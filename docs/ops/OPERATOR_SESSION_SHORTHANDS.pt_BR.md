...
Exemplos versionados e scripts usam o alias SSH **`latitude`** para o servidor Linux do lab (Zorin; Docker, reports). Configure **`Host latitude`** no **`~/.ssh/config`** do PC de desenvolvimento para coincidir com **DNS/mDNS na LAN** (e chaves **ed25519** já autorizadas no host). Ver **`docs/private.example/homelab/README.md`**.
