...
- **Supply Colleague-Nn:** confirma que **`uv.lock`** continua **sem `litellm`** depois de qualquer refresh de deps; nota de uma linha em **`SECURITY.md`** / diário privado sobre o incidente PyPI **`litellm`** (mar/2026) se quiseres lembrete durável (opcional).
