...
| 2. **Merge** | **Version bump PR** (build `1.6.3` → `1.6.4` per [VERSIONING.md](../VERSIONING.md)) so `pyproject.toml` on `main` is the version you are releasing |
...
- **Pros:** Hub **`fabioleitao/data_boar:1.6.4`** matches **About / reports**; operators can trust tag = app version; one story for support.
...
- **Cons:** Short window where **`:latest`** digest was built from **`main` still at 1.6.3** — confusing if someone checks **About** vs **Hub tag** the same day. Mitigation: bump **within the same session** or document “pre-release digest”.
