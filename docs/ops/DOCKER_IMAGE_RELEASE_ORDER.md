...
| 5. **Smoke** | **Required before Hub push:** short `docker run --rm` against **`data_boar:lab`** (e.g. `python main.py --version` or `/health`) — see [DOCKER_SETUP.md](../DOCKER_SETUP.md) §7 / [DEPLOY.md](../deploy/DEPLOY.md). **Do not push** if smoke fails. |
