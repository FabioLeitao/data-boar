...
| 2. **Merge** | **Version bump PR** (build → **`1.6.6`** per [VERSIONING.md](../VERSIONING.md)) so `pyproject.toml` on `main` is the version you are releasing                                     |
...
- **Pros:** Hub **`fabioleitao/data_boar:1.6.6`** (example) matches **About / reports**; operators can trust tag = app version; one story for support.
