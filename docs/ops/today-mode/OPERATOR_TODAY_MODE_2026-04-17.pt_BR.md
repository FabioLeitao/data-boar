...
**Tema:** Confirmar que **`main`** está **verde no GitHub Actions** após o ajuste do **`ci.yml`** (condição com `secrets` no nível do job removida). **Puxar** o remoto. **v1.7.0** + amostra **FCPA** + **dicas de jurisdição** como linha de base **beta estável** para **Colleague-A** e **Colleague-B** iniciarem **testes sintéticos / estruturados**, e para **lab-op** quando quiseres. **Triage** do carryover do **2026-04-16** sem perder o ritmo **Corporate-Entity-C / WRB**.
