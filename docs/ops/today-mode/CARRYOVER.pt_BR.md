...
| ---- | ------ | ------ | ----- |
| **Harness `benchmark-ab` na `main` (PR #229)** — `scripts/benchmark-ab.ps1`, gitignore `benchmark_runs/`, runbook/hub, `docs/plans/BENCHMARK_EVOLUTION.md` | EOD **2026-04-27** (carryover do ritual completão / benchmark) | ✅ Mergeado (CI verde) | **Manhã:** `git fetch origin` + `git checkout main` + `git pull origin main` (neste clone o merge terminou em **`6c9ef7e`**); com janela LAB, correr `.\scripts\benchmark-ab.ps1` se precisar e substituir linhas **TBD** em `BENCHMARK_EVOLUTION.md` por `times.txt` real + deltas de manifest. |
