...
> Sessão anterior (madrugada): contexto no chat + atualizações em `docs/private/` (mapa mental, goals, sync carreira Corporate-Entity-A→Corporate-Entity-B).
...
- [ ] **LAB-NODE-01 / `uv`:** playbook actual não instala toolColleague-Nn dev — se quiseres testar Data Boar **a partir do código** no LAB-NODE-01, ou instalas `uv` à mão ou abrimos issue/PR para role opcional `uv` (discutido no chat)
...
- [ ] LinkedIn: typo **Securiy** → **Security**; About alinhado (Corporate-Entity-B atual; Corporate-Entity-A passado) — checklist em `FABIO_GOALS_MASTER.pt_BR.md`
...
- **Corporate-Entity-B:** política de LinkedIn até kit físico — mantém-se conforme notas anteriores.
