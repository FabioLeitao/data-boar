...
**Purpose:** Treat **WSL2** on the primary Windows dev machine (typical ThinkPad-class laptop) as an **optional second execution surface** alongside native Windows and alongside Linux laptops (T14, latitude). This does **not** replace CI or homelab hosts; it reduces the risk of assuming a single OS or a single container runtime for how customers or integrators run Data Boar.
