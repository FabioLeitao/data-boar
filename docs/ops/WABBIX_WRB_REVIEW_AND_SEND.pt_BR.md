...
**Nome interno:** **WRB** (*Corporate-Entity-C Review Briefing*).

Este ficheiro complementa o guia canónico [`Corporate-Entity-C_REVIEW_REQUEST_GUIDELINE.md`](Corporate-Entity-C_REVIEW_REQUEST_GUIDELINE.md): aqui tens a **matriz de conformidade** (o que o guideline e o *long form* pedem vs o que o prompt PT-BR cobre), o **texto pronto para colar** no e-mail (espelho do bloco “Prompt mestre completo”) e uma **versão curta para WhatsApp**.

**Fonte de verdade do texto longo:** o bloco ` ```text ` em `Corporate-Entity-C_REVIEW_REQUEST_GUIDELINE.md` — se divergir, prevalece o guia; atualiza este espelho quando alterares o prompt mestre.

**Notas in-repo alinhadas ao último ciclo:** `docs/plans/Corporate-Entity-C_ANALISE_2026-03-18.md` e referências em `docs/plans/PLANS_TODO.md` (secção Corporate-Entity-C).

### Ao criar um pedido WRB no futuro

1. **Mostrar na tela** o texto completo (chat/IDE) para revisão antes de enviar à Corporate-Entity-C.
2. **Guardar no repositório** o mesmo conteúdo: atualizar o bloco “Prompt mestre completo” em [`Corporate-Entity-C_REVIEW_REQUEST_GUIDELINE.md`](Corporate-Entity-C_REVIEW_REQUEST_GUIDELINE.md) e, se usares este pacote, a secção §2 abaixo; fazer **`git commit`** para não perder o rascunho se a sessão ou o histórico sumirem.
...
| Caminho da nota de tracking | **Sim** (`Corporate-Entity-C_ANALISE_2026-03-18.md`) |
...
| --------------------------- | ------------ |
| Não confundir acumulado / último relatório Corporate-Entity-C / última release | **Sim** |
...
Use quando o canal for curto; o pedido completo continua no repositório (`Corporate-Entity-C_REVIEW_REQUEST_GUIDELINE.md`, secção “Prompt mestre completo”). Podes anexar o PDF local ou colar o texto longo numa mensagem de seguimento.

```text
Olá! Pedido WRB — Data Boar: novo relatório de revisão, por favor.

Contexto rápido: último relatório de vocês 2026-03-18 (9,1/10 nas nossas notas); tracking em docs/plans/Corporate-Entity-C_ANALISE_2026-03-18.md. No início do PDF: por favor indiquem data de referência + PR/commit-base que usaram na comparação.
...
O briefing completo em prosa está em docs/ops/Corporate-Entity-C_REVIEW_REQUEST_GUIDELINE.md — podem seguir esse ficheiro como fonte.
