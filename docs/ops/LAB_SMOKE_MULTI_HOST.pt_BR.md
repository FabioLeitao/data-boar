...
| ----- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| A     | **Desbloquear git** no LAB-NODE-01, lab-node-02, LAB-NODE-03: `main` precisa permitir fast-forward a `origin/main` (ou documentar exceção).                                        | `git pull --ff-only` com sucesso. <-feito, vai se virar de novo daqui...                                                                                                                      |
| B     | **LAB-NODE-04:** libertar espaço em disco; depois `git fetch` funciona.                                                                                                    | Sem `No space left on device` no fetch. <-feito, vai se virar de novo daqui...                                                                                                                |
| C     | No **LAB-NODE-01** **e** num **segundo** hub (ex.: **lab-node-02**): `deploy/lab-smoke-stack` → `docker compose up -d`; healthchecks OK **nos dois**.                          | `docker compose ps` saudável em cada um; TCP `55432` / `33306` a partir da LAN para cada IP de hub. <-feito, vai se virar de novo daqui...                                                    |
...
| F     | **lab-node-02:** `uv run` scan para o mesmo host:portas; dashboard opcional `:8088`.                                                                                   | Igual a E a partir de caminhos Linux. <-feito, vai se virar de novo daqui...                                                                                                                  |
| G     | **LAB-NODE-03:** scan CLI pela LAN; confirmar wheels **musl** (PyMySQL/psycopg2).                                                                                       | Sem erros de import; scan concluído. <-feito, vai se virar de novo daqui...                                                                                                                   |
| H     | **LAB-NODE-04:** `scan.max_workers: 1`; último na cadeia.                                                                                                                  | Conclui ou documenta timeout/OOM para o runbook. <-feito, vai se virar de novo daqui...                                                                                                       |
