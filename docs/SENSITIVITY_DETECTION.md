...
| `sensitivity_detection.checksum_validation`                     | Boolean, default **false**. When **true**, regex shape matches for **CPF** (`LGPD_CPF`) and **credit card** (`CREDIT_CARD`) are post-filtered with the existing modulo-11 (`core.brazilian_cpf.cpf_checksum_valid`) and Luhn modulo-10 (`core.luhn.luhn_check`) helpers before reaching the **HIGH** branch. Random 11-digit IDs and 16-digit barcodes that only matched on shape no longer count as strong PII; instead the detector returns **MEDIUM** with `pattern_detected` **`CHECKSUM_REJECTED:LGPD_CPF`** (or `:CREDIT_CARD`) and `norm_tag` *“Shape match without valid check digit”* — a truthful diagnostic per the *defensive scanning manifesto* and *art of the fallback*. Email and other patterns are unaffected. **Off by default** to preserve current FN-first behaviour; see [ADR 0044](adr/0044-opt-in-checksum-validation-cpf-credit-card.md). |                                                                              |
