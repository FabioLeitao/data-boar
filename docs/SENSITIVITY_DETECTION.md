...
| `detection.quasi_identifier_mapping`          | list    | **[]**   | Optional list of `{ column_pattern, category }` or `{ pattern_detected, category }` to map columns/patterns to `gender`, `job_position`, `health`, `address`, `phone`, `other`. Built-in defaults map common names (e.g. gender, sex, cargo, department, health, address) and **phone** under multiple naming schemes and languages: phone, telefone, celular, mobile, home phone, work phone, cell phone, téléphone, teléfono, handy, handynummer, mobilnummer, etc. |
