...
**Compliance samples (one profile per regulation):** Sample configs for **UK GDPR**, **PIPEDA**, **POPIA**, **APPI**, and **PCI-DSS** are in [compliance-samples/](compliance-samples/). Each sample is a single YAML file with regex patterns, ML terms, and recommendation overrides for that framework. Use them as follows:
...
**Available sample:** [compliance-sample-uk_gdpr.yaml](compliance-samples/compliance-sample-uk_gdpr.yaml) – UK GDPR (UK post-Brexit, ICO-aligned). More samples (PIPEDA, POPIA, APPI, PCI-DSS) will be added in the same format; see the [compliance-samples README](compliance-samples/README.md).
