...
- [ADR 0044](0044-opt-in-checksum-validation-cpf-credit-card.md) (EN) — **gate algorítmico opcional** para **CPF** (módulo-11) e **cartão de crédito** (Luhn) no detector: flag `sensitivity_detection.checksum_validation`; demoção transparente para **MEDIUM** com `pattern_detected = CHECKSUM_REJECTED:<família>` quando o check digit falha; helper único `core.luhn` reutilizado por `pro/worker_logic`.
