...
- [ADR 0044](0044-resilience-retry-backoff-circuit-breaker.md) (EN) — **resiliência por target**: retry com **backoff exponencial + jitter** (ladder padrão **1 s → 4 s → 10 s**) e **circuit breaker** por target (3 falhas → trip; cooldown de **15 min**; estado HALF_OPEN deixa passar uma sondagem). Engine pula targets com circuito **OPEN** sem matar o scan; telemetria em `audit_log.circuit_breakers`; chave YAML `circuit_breaker:` por target + envs `DATA_BOAR_CIRCUIT_*` / `DATA_BOAR_RETRY_*`. Honra `DEFENSIVE_SCANNING_MANIFESTO.md` (não pisar no DB do cliente) e `THE_ART_OF_THE_FALLBACK.md` (toda demoção deixa rastro).
