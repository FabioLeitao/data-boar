...
- [ADR 0044](0044-runtime-feature-gate-facade-for-open-core-boundary.md) (EN) — **feature-gate de runtime** unificado (`core/licensing/feature_gate.py`) ligando `LicenseGuard` → tier → registro de features; tier **OPEN** continua liberando tudo (default da comunidade); decisões retornam motivo factual para o `scan_audit_log` (alinhado ao **THE_ART_OF_THE_FALLBACK** §3 e ao **DEFENSIVE_SCANNING_MANIFESTO** — gate **não** mexe em sampling caps / NOLOCK).
