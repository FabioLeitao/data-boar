...
- [ADR 0044](0044-container-image-hardening-databoar-user.md) (EN) — **endurecimento da imagem de contêiner**: usuário não-root **`databoar`** (uid 1000), **`pip`/`wheel`** removidos do estágio final, `HEALTHCHECK` na imagem e rótulos OCI; Compose pinado com `read_only: true`, `cap_drop: [ALL]`, `no-new-privileges:true`, `user: "1000:1000"`, `pids_limit` e tmpfs em `/tmp`; **zero impacto** no caminho de amostragem SQL (Defensive Scanning Manifesto preservado); migração para Distroless / Wolfi fica como slice futuro.
