...
Este guia resume **como trabalhar em conjunto** (ex.: **Fabio** como mantenedor e **Contributor-A** como colaborador) com **coesão**: fluxo Git, identidade nos commits, limites do assistente no Cursor e **o que configurar** no repositório (rules, `AGENTS.md`, hábitos).
...
| Mecanismo | Uso |
| --------- | --- |
| **Issues / Projects no GitHub** | Assignee **Fabio** vs **Contributor-A**; labels `owner:maintainer`, `owner:contributor` (ou nomes que preferirem). |
| **Nomes de branch** | Ex.: `contributor-a/descricao-curta`, `fabio/descricao-curta` ou só `feature/…` com assignee na PR. |
| **`docs/private/`** (gitignored) | Notas **só do mantenedor** / LAB-OP **reais** — **não** entram no clone público do colaborador da mesma forma; evita misturar “chores” pessoais com o que o Contributor-A precisa ler. |
...
### 4.1 Colaborador (Contributor-A) — clone do **fork** (primeira vez)

```bash
git clone https://github.com/Contributor-A_USER/nome-do-fork.git
cd nome-do-fork
...
git checkout -b contributor-a/minha-feature
```
...
git push origin contributor-a/minha-feature
```
...
### 4.6 Mantenedor — adicionar remote do fork do Contributor-A (opcional, para `fetch` da branch dele)

```bash
git remote add contributor-a https://github.com/Contributor-A_USER/nome-do-fork.git
git fetch contributor-a
git checkout -b review/contributor-a-feature contributor-a/contributor-a/minha-feature
```

(Ajustar nome da branch conforme o Contributor-A empurrou.)
...
git checkout -b contributor-a/minha-feature
# ... commits ...
git push -u origin contributor-a/minha-feature
```
...
> Estou no papel de **contribuidor** (Contributor-A). Branch atual: `contributor-a/…`. O mantenedor é o Fabio. Segue [AGENTS.md](AGENTS.md) e [CONTRIBUTING.pt_BR.md](../CONTRIBUTING.pt_BR.md). Não assumas acesso a `docs/private/`. Proponha alterações em commits pequenos e compatíveis com PR único para a feature X.
...
> Esta tarefa é **só para o colaborador Contributor-A** (documentação de onboarding). Não mistures com itens de homelab ou estudo do mantenedor.
...
| Artefato | Função |
| -------- | ------ |
| **[`AGENTS.md`](../AGENTS.md)** | Contrato global do assistente: idioma, segredos, homelab, Git/PR, testes. **Ambos** devem ler uma vez; o Contributor-A segue o mesmo ficheiro ao clonar. |
...
| Data | Nota |
| ---- | ---- |
| 2026-03-01 | Versão inicial (Fabio + Contributor-A, fork ou repo único, comandos, prompts, rules). |
