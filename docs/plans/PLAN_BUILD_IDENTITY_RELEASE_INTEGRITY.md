...
| # | To-do | Notes |
| - | ----- | ----- |
| E.11 | **`--export-audit-trail [PATH]`** on **`main.py`**: writes JSON (default **stdout** if omitted or **`-`**). Current payload: `schema_version`, `exported_at`, **`application`** (from `get_about_info()`), **`paths`**, **`data_wipe_log`** (full table), **`scan_sessions_summary`**, placeholders **`integrity_anchor`** / **`integrity_events`** for Phase E persistence. | Does **not** open a scan or call wipe. Incompatible with **`--web`** / **`--reset-data`**. |
