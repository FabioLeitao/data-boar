...
| **Backlog**      | Risco de quasi-identificação (IP+geo+breadcrumbs)                                 | A/M                                   | [PLAN_QUASI_IDENTIFICATION_COMPOSITE_RISK.md](PLAN_QUASI_IDENTIFICATION_COMPOSITE_RISK.md)                                                                                                                                                                   |
...
| **S2 – Profundidade de detecção**     | FN / hints            | Uma ou duas linhas do plano FN (ex. 5, 6 ou 10–11) **ou** uma fatia de quasi-identificação (score composto/guardrails em relatório)—**não todas**                      | Revisar config em dado real se preciso                           | Implementação + testes + docs SENSITIVITY_DETECTION                  |
...
| **M-QID**           | Controles de quasi-identificação ativos                               | Score composto + thresholds + fatores explicáveis + saída de relatório com redação para combinações IP+geo+breadcrumbs; enriquecimento externo desligado por padrão e só com opt-in explícito + limites.                                                                                                                                                                                                                                                                                                                                                                                                                                            |
...
**Cadência sugerida:** **M-TRUST** antes de rajada grande de feature; **M-OBS** pode vir no mesmo sprint ou no seguinte a **M-TRUST** (docs/automação pequena); **M-LAB** antes de narrativa para cliente/demo; **M-QID** junto aos ciclos de profundidade de detecção quando logs/proxy estiverem no escopo; **M-RICH** quando o PR de rich media entrar em `main`; **M-ACCESS** antes de prometer **assinatura permanente** ou **multiusuário** em host alcançável; **M-SITE-READY** quando **publicarem de propósito** a superfície marketing/docs (faz sentido depois de **M-LOCALE-V1** se quiserem UX de idioma coerente); **M-RELEASE** quando VERSIONING mandar publicar.
