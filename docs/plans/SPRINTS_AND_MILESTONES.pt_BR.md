...
**POC de maturity GRC (URLs relacionadas, tema distinto):** O trabalho de questionário organizacional ([PLAN_MATURITY_SELF_ASSESSMENT_GRC_QUESTIONNAIRE.md](PLAN_MATURITY_SELF_ASSESSMENT_GRC_QUESTIONNAIRE.md)) compartilha `api/routes.py` / templates com o painel, mas tem **backlog próprio** (rubrica, export, depois **histórico por envio** / **tier** / **ganchos RBAC** quando a **história de produto** fechar). **Sequência:** levar essa fatia do POC a um ponto de controlo coerente **antes** de priorizar a **[#86](https://github.com/FabioLeitao/data-boar/issues/86) Fase 1** num **branch separado** — a Fase 1 continua **o mais cedo possível** **depois** desse ponto, não “em vez de” fechar o ímpeto do POC. Cruzamento: [PLANS_TODO.md](PLANS_TODO.md) *Integration / WIP*, bullet *Maturity self-assessment*.
...
| 3     | **Seguimento POC maturity**           | A/M  | **Depois** do merge de rubrica + export: **histórico**, **alinhamento por tier**, **ganchos RBAC** conforme narrativa de produto — mesmo plano de maturity; **antes** da linha 4 se o POC for o tema ativo. |
