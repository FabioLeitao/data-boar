...
**Version signal for testers (do not rely on git inference alone):** in-repo semver is **`1.7.1`** ([VERSIONING.md](../VERSIONING.md)); **last stable** published story is **1.7.1** (Hub / GitHub Release). Callouts: [CHANGELOG.md](../../CHANGELOG.md), [docs/releases/1.7.1.md](../releases/1.7.1.md), README **Current release**. Anyone pulling `main` for **formal** testing should use those files—not only `git log`.
