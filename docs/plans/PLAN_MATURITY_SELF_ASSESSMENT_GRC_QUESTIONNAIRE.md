...
**Version signal for testers (do not rely on git inference alone):** in-repo semver on **`main`** is **`1.7.2-beta`** ([VERSIONING.md](../VERSIONING.md) `-beta`); **last stable** published story is **1.7.1** (Hub / GitHub Release). Callouts: [CHANGELOG.md](../../CHANGELOG.md), [docs/releases/1.7.1.md](../releases/1.7.1.md), README **Current release** vs **`main` pre-release**. Anyone pulling `main` for **formal** beta testing should use those files—not only `git log`.
