...
| **1** | **Assistant-safe access** | API Integration **`.env`** (gitignored), TLS trust for `https://unifi.local` (or your hostname), smoke + inventory scripts succeed from Cursor terminal. | `scripts/udm-api-smoke-from-env.ps1`, `scripts/udm-api-inventory-from-env.ps1` (optional `-SaveJson` to private `reports/`); no keys in git or chat |
