...
| **1** | **Acesso seguro ao assistente** | `.env` API (gitignored), confiança TLS para `https://unifi.local` (ou hostname), scripts smoke + inventário OK no terminal do Cursor. | `scripts/udm-api-smoke-from-env.ps1`, `udm-api-inventory-from-env.ps1` (opcional `-SaveJson` em `reports/` privado); sem chaves no git nem no chat |
