...
- **Session shorthand:** Taxonomy keywords (`deps`, `feature`, `homelab`, `docs`, `houseclean`, `backlog`, `pmo-view`, …) are **English-only** in chat and in **`.cursor/rules/session-mode-keywords.mdc`**—see **`AGENTS.md`**. Grouping + LAB-OP SSH naming (**`lab-node-02`**): **`docs/ops/OPERATOR_SESSION_SHORTHANDS.md`** ([pt-BR](../ops/OPERATOR_SESSION_SHORTHANDS.pt_BR.md)).
