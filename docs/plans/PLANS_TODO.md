...
| Quasi-identification composite risk (IP+geo+breadcrumbs) | Optional: Additional detection techniques & FN reduction                                                                     | None           | Composite score for re-identification/deanonymization risk in logs/proxy traces; safe defaults (local-first, no online enrichment by default), redaction and operator review gates. See PLAN_QUASI_IDENTIFICATION_COMPOSITE_RISK.                                            |
...
| 3a    | **Quasi-identification composite risk (IP+geo+breadcrumbs)**       | New focused vertical for logs/proxy deanonymization risk scoring with LGPD guardrails. Start with local-only scoring + report labels + redaction, then optional enrichment adapter behind explicit opt-in. See [PLAN_QUASI_IDENTIFICATION_COMPOSITE_RISK.md](PLAN_QUASI_IDENTIFICATION_COMPOSITE_RISK.md).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
...
**LAB-OP — CAPEX/OPEX & procurement spine:** ✅ **Tracked plan created** — [PLAN_LAB_OP_CAPEX_OPEX_AND_PROCUREMENT.md](PLAN_LAB_OP_CAPEX_OPEX_AND_PROCUREMENT.md) centralizes priorities, gates, opportunities, and rationale **without prices**. **Private source of truth (real values):** `docs/private/homelab/LAB_OP_CAPEX_OPEX_PROCUREMENT_MASTER.pt_BR.md` (gitignored; template in `docs/private.example/homelab/`).
...
1. **Quasi-identification composite risk (IP+geo+breadcrumbs) – first slices** *(mixed, LGPD guardrails first)*
   - Plan: [PLAN_QUASI_IDENTIFICATION_COMPOSITE_RISK.md](PLAN_QUASI_IDENTIFICATION_COMPOSITE_RISK.md).
   - Start with: local-only composite scoring + thresholds + explainable factors + redacted report output.
   - Then optional: external enrichment adapter only behind explicit opt-in, bounded lookups, and audit metadata.
