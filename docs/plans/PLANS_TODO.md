...
- **Slice 5 — Enterprise Hardening (RFC, doc-only):** Roadmap critique against Russinovich / Charity Majors enterprise bar named **six gaps** (G1–G6): external secrets manager (AWS SM / HashiCorp Vault / Azure KV), multi-tenant evidence isolation, automated post-mortem on failed scans, opt-in OTel performance telemetry, **agentless posture** that the README does not yet state loudly, and a concrete review-packet template for M-TRUST-04. Slice 5 packages these as **five phases** (5.1–5.5) with **doc-first** artifacts and **thin code slices** to be promoted later — full RFC: [PLAN_SLICE_5_ENTERPRISE_HARDENING.md](PLAN_SLICE_5_ENTERPRISE_HARDENING.md). **Sequencing:** runs **after S2a** (Transport + trust POC) closes; **5.5** (agentless positioning, doc-only) may proceed **in parallel** with S2a as a `pauseable` sidequest. **Effort vs stakeholder value re-order:** **5.5 → 5.3 → 5.1 → 5.2 → 5.4** (highest CISO recall first; OTel last because it adds an optional dep tree). **Zero impact on customer DB locks** (RFC is doc-only; promoted phases each preserve sampling caps, fallback hierarchy, and audit-log truthfulness per [DEFENSIVE_SCANNING_MANIFESTO.md](../ops/inspirations/DEFENSIVE_SCANNING_MANIFESTO.md) and [THE_ART_OF_THE_FALLBACK.md](../ops/inspirations/THE_ART_OF_THE_FALLBACK.md)).
