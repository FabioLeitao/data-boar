...
- **Status rows counted:** 149  (Done: 84 | Incomplete: 65)
...
### Integration / WIP — last refreshed **2026-04-21**
...
- **Scope import / inventory bootstrap (roadmap):** Not implemented — phased plan [PLAN_SCOPE_IMPORT_FROM_EXPORTS.md](PLAN_SCOPE_IMPORT_FROM_EXPORTS.md): canonical import format → merge-safe **config fragments** from **offline exports** (assessment tools such as Live Optics when used as a provider, ITSM such as GLPI, classic monitoring such as Nagios/Cacti, observability breadcrumbs from Prometheus/Datadog/Dynatrace-class **exports**). Complements opt-in port hints (**passive seed** vs **active** probes). Positions Data Boar closer to **customer teams and existing toolColleague-Nn** without replacing CMDB or observability stacks.
- **Maturity self-assessment / LGPD questionnaire (operator artefacts → product backlog):** Under consideration — [PLAN_MATURITY_SELF_ASSESSMENT_GRC_QUESTIONNAIRE.md](PLAN_MATURITY_SELF_ASSESSMENT_GRC_QUESTIONNAIRE.md) — **not** in active code yet; **M-LOCALE-V1** is the **current** primary `feature` slice; **GRC POC scaffolding** is explicitly **deferred to the following `feature` session** (option **A** first, then **B → C → D** per plan). Private: `docs/private/raw_pastes/general/LGPD_DOCS/`. **Commercial / subscription** feature—not open core.
...
| Dashboard i18n                                 | **D-WEB** ✅; **M-LOCALE-V1 promoted** before [#86](https://github.com/FabioLeitao/data-boar/issues/86) Phase 1 code                              | None           | **Path prefix + JSON + cookie/`Accept-Language`/fallback**; **~5 locales** long-term; gettext backlog. Ship **M-LOCALE-V1** first, then **#86** Phase 1 (session/passwordless) on **prefixed** paths — [PLAN_DASHBOARD_I18N.md](PLAN_DASHBOARD_I18N.md). |
...
1. **Dashboard i18n** – **target architecture locked** in [PLAN_DASHBOARD_I18N.md](PLAN_DASHBOARD_I18N.md); **D-WEB** ✅. **Promoted:** **M-LOCALE-V1** before [#86](https://github.com/FabioLeitao/data-boar/issues/86) **Phase 1** (avoid auth on legacy unprefixed routes). Coordinate **#86** on the same `/{locale}/…` layout.
...
**Status:** **D-WEB** ✅ — **M-LOCALE-V1** **promoted** before [#86](https://github.com/FabioLeitao/data-boar/issues/86) Phase 1 (session/passwordless).

| Milestone         | Purpose                                                                                             | When                            |
| ---------         | -------                                                                                             | ----                            |
| **D-WEB**         | URL map + **middleware order** — **doc / diagram**; cross-link **#86** plan                       | ✅ Done (Phase 0)               |
| **M-LOCALE-V1**   | Path-prefixed HTML; **`en` + `pt-BR`** JSON; negotiation; switcher; tests; CI locale key parity     | **Next** implementation slice   |
...
**RBAC (#86):** Phase **1+** **after** **M-LOCALE-V1** on **prefixed** paths — [PLAN_DASHBOARD_REPORTS_ACCESS_CONTROL.md](PLAN_DASHBOARD_REPORTS_ACCESS_CONTROL.md) § *Sequencing with dashboard i18n*.
