...
| **mini-bt** | Mini PC Void Linux | utility / proxy / gateway | 8 GiB RAM, SATA SSD, zram 2 GB | uv + Python 3.14 — bem equipado para tarefas leves |
...
| ---- | ------ | ----- | ------------------- |
| UDM | (ver private) | firewall + router + IDS/IPS | único ponto de falha; config backup periódico crítico |
| Switch | (ver private) | L2 | sem redundância de link |
| APs | UniFi | Wi-Fi | dependentes do UDM |

### 2.2 Melhorias de rede (P0/P1/P2)

| Item | Prioridade | Justificativa |
| ---- | ---------- | ------------- |
| **Backup de config UDM/switch/AP** (automatizado) | P0 | Sem backup, qualquer falha de hardware = horas de reconfiguração. |
...
| **UPS para rede** (UDM + switch + ONT/modem) | P0 | Sem UPS, uma queda de energia derruba toda a LAN mesmo que os servidores sobrevivam. |
| **UDM reserva** (cold spare) | P1 | Swap em minutos vs dias esperando entrega. |
...
| **UPS para hosts críticos** (T14 + servidor) | P1 | Shutdown limpo evita corrupção de FS (BTRFS, ZFS). |
...
| Item | Prioridade | Justificativa |
| ---- | ---------- | ------------- |
| Verificar suporte nativo (T14 Goodix `27c6:6594` já detectado) | P0 | Goodix já aparece no USBGuard como `allow`; verificar `libfprint` support. |
...
1. **RAM T14** (16→32 GiB, DDR5 SODIMM): slot livre confirmado, custo baixo vs impacto alto.
2. **UPS para rede** (UDM + switch): sem isso, qualquer queda derruba tudo.
3. **Backup automático de config UDM**: risco alto, custo zero (script).
...
10. **Servidor x86 para Proxmox** (aposentar Latitude E6430).
11. **UPS para hosts críticos** (T14 + servidor).
...
Fase 1 (agora, baixo custo):
  RAM T14 → UPS rede → backup UDM → SSD externo → kit manutenção
...
Fase 3 (crescimento):
  Solar (expansão) → Pi 4/5 → switch 2.5G → UDM spare
