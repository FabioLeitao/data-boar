...
**Realidade de hardware:** LAB-NODE-01 com **≤16 GB RAM** não deve correr Prometheus + Loki + Graylog + OpenSearch + Wazuh + k3s **em simultâneo**. Escolhe **um** caminho de métricas e **um** de logs; stacks pesados no **torre/VM Proxmox** quando existir.
...
**Evitar no mesmo LAB-NODE-01 em simultâneo:** Graylog + OpenSearch + Prometheus completo + Loki + Wazuh + k3s + backend de traces. Escolhe **A ou B**, **C ou D**, **E** quando houver recursos, **F** quando traces justificarem a RAM (muitas vezes **VM** à parte).
