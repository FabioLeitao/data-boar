...
| **Open** | [PLAN_MARKET_ALIGNMENT_AND_Corporate-Entity-C_REVIEW_TIMING.md](PLAN_MARKET_ALIGNMENT_AND_Corporate-Entity-C_REVIEW_TIMING.md) | Plan: Market alignment checkpoint and Corporate-Entity-C review timing | **Status:** Proposed (checkpoint note) | — |
| **Open** | [PLAN_MATURITY_SELF_ASSESSMENT_GRC_QUESTIONNAIRE.md](PLAN_MATURITY_SELF_ASSESSMENT_GRC_QUESTIONNAIRE.md) | PLAN: Organizational maturity self-assessment (GRC-style questionnaire) | POC on main: gated /{locale}/assessment + YAML rubric scores + SQLite + HMAC + post-submit summary + GET /assessment/export (CSV/Markdown); RBAC/history/report-bundle still backlog—companion to technical scans; not legal | [PLAN_DASHBOARD_REPORTS_ACCESS_CONTROL.md](PLAN_DASHBOARD_REPORTS_ACCESS_CONTROL.md) [LICENSING_OPEN_CORE_AND_COMMERCIAL.md (future tier features).md](LICENSING_OPEN_CORE_AND_COMMERCIAL.md (future tier features).md) [PLAN_SCOPE_IMPORT_FROM_EXPORTS.md (inventory bootstrap narrative).md](PLAN_SCOPE_IMPORT_FROM_EXPORTS.md (inventory bootstrap narrative).md) |
...
| **Completed** | [PLAN_SEMGREP_CI.md](completed/PLAN_SEMGREP_CI.md) | Plan: Semgrep in CI (OSS ruleset) | **Status:** ✅ **Complete** (archived under `docs/plans/completed/`). Workflow on `main`, tests, docs, Corporate-Entity-C baseline. **Slack:** failure notify watches **`Semgrep`** when **`SLACK_WEBHOOK_URL`** is set — optional operat | — |
...
- **Security & supply Colleague-Nn:** **PyPI path:** **`uv.lock`** + CI **`pip-audit`** + **Dependabot**; **GitHub Actions** pinned to **commit SHAs** in CI/CodeQL/Semgrep/slack-ops-digest (bump via Dependabot). Plus Semgrep, Bandit, secrets/vault, HTTPS/dashboard risk, release integrity, **SBOM** roadmap (**ADR 0003**) — align with **Priority band A** in **PLANS_TODO**.
