...
1. **Data type / length hint** from connectors → optional “format hint” in detector and MEDIUM suggestion when format suggests identifier but no regex hit — **done (ongoing slices):** `sensitivity_detection.connector_format_id_hint` + `FORMAT_LENGTH_HINT_ID` / `FORMAT_TYPE_HINT_ID_INT` / `FORMAT_LENGTH_HINT_EMAIL`; SQL `CHAR`/`VARCHAR(N)` length parsed from connector metadata; conservative name heuristics (CPF/CNPJ/SSN; **UUID** `VARCHAR(36)` / hex `VARCHAR(32)` when column name contains `uuid`, `guid`, or `uniqueidentifier`); wired through `DataScanner.scan_column` / SQL, Snowflake, Dataverse, Power BI, SQLite-in-file path in filesystem connector. **Email-length hint** extended to common schema sizes (**128, 191, 254, 255, 256, 320**) with existing email-like name tokens. **REST/API** connector passes inferred `VARCHAR(n)` / `BIGINT` from JSON scalar samples (`connectors/rest_connector.py`). Further work: MongoDB/Redis key-value typing hints if needed.
