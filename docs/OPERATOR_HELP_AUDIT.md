...
- [x] **OpenAPI** vs `POST /scan` / `POST /start` — request body model is **`ScanStartBody`** in **`api/routes.py`**: optional `tenant`, `technician`, `scan_compressed`, `content_type_check` (all nullable booleans/strings as per model). FastAPI serves the generated schema at **`/openapi.json`** / **`/docs`** when `--web` is running — spot-check there after schema changes; USAGE documents scan behaviour.
