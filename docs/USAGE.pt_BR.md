...
- `file_scan` – extensões, recursividade, `scan_sqlite_as_db`, `sample_limit`, opcionalmente `scan_compressed` (varrer dentro de arquivos compactados, ex.: zip, tar, 7z — desligado por padrão), limites de tamanho (`max_inner_size`) e `compressed_extensions`, e `file_passwords` (senhas para PDFs e documentos ZIP protegidos por senha; ver [USAGE.md](USAGE.md), seção Global options, para exemplo YAML e limitações). Para suporte a .7z instale o extra opcional: `pip install -e ".[compressed]"`.
