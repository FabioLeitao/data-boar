...
- **Resiliência por target — retry com jitter e circuit breaker (database / Snowflake / Mongo):** o `connect()` roda dentro de um **circuit breaker** por target (`core/resilience.py`). Erros transitórios de driver (TCP reset, statement timeout, failover de réplica, `OperationalError`, `ServerSelectionTimeoutError`, …) são repetidos com **backoff exponencial e jitter completo** — ladder padrão **1 s → 4 s → 10 s**. Após **3** falhas consecutivas o circuito **trip**: novas tentativas no mesmo target falham imediatamente com uma linha de falha estruturada (**`reason=circuit_open`**), o motor **pula esse target** e segue com os outros. Após **15 minutos** de cooldown, o próximo acesso entra em HALF_OPEN — uma sondagem é liberada; sucesso fecha o circuito, falha re-abre. Override por target (chave YAML opcional `circuit_breaker:` com `failure_threshold`, `cooldown_seconds`, `retry_attempts`, `retry_max_seconds`, `jitter`) e variáveis de processo (`DATA_BOAR_CIRCUIT_FAILURE_THRESHOLD`, `DATA_BOAR_CIRCUIT_COOLDOWN_SECONDS`, `DATA_BOAR_RETRY_ATTEMPTS`, `DATA_BOAR_RETRY_MAX_SECONDS`) ficam disponíveis para resposta a incidente. Telemetria do breaker (estado, contagem de falhas, segundos de retry-after, classe da última exceção) é exposta em `audit_log.circuit_breakers` para o dashboard / API. Erros de programa (`ValueError`, `TypeError`, `KeyError`) **nunca** disparam retry e **nunca** abrem o circuito. Ver **ADR 0044** e o `DEFENSIVE_SCANNING_MANIFESTO.md` para o contrato.
