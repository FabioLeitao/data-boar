...
2. **Don’t be too aggressive:** Too-low timeouts cause false timeouts on busy or slow networks (e.g. during backup). If you see many timeouts, **increase** `connect_timeout_seconds` and `read_timeout_seconds` (or per-target overrides) and consider re-running during off-peak.
