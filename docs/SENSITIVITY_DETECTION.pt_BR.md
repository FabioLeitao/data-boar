...
| `sensitivity_detection.checksum_validation`                     | Booleano, padrão **false**. Se **true**, achados regex de **CPF** (`LGPD_CPF`) e **cartão de crédito** (`CREDIT_CARD`) passam por validação algorítmica (módulo-11 em `core.brazilian_cpf.cpf_checksum_valid` e Luhn módulo-10 em `core.luhn.luhn_check`) antes de chegar ao ramo **HIGH**. IDs aleatórios de 11 dígitos e códigos de barras de 16 dígitos que casaram só pelo formato deixam de virar PII forte por shape; em vez disso, o detector retorna **MEDIUM** com `pattern_detected` **`CHECKSUM_REJECTED:LGPD_CPF`** (ou `:CREDIT_CARD`) e `norm_tag` *“Shape match without valid check digit”* — diagnóstico verdadeiro alinhado ao *defensive scanning manifesto* e ao *art of the fallback*. E-mail e demais padrões não são afetados. **Desligado por padrão** para preservar o comportamento atual (FN-first); ver [ADR 0044](adr/0044-opt-in-checksum-validation-cpf-credit-card.md). |                                                                                |
