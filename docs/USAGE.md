...
**Resilience — retry and circuit breaker (database / Snowflake / Mongo targets):** Connect calls run under a per-target **circuit breaker** (`core/resilience.py`). Transient driver errors (TCP resets, statement timeouts, replica failovers, `OperationalError`, `ServerSelectionTimeoutError`, …) are retried with **exponential backoff and full jitter** — default ladder **1 s → 4 s → 10 s**. After **3** consecutive failures the breaker **trips**: subsequent attempts on that target short-circuit immediately with a structured `circuit_open` failure row, the engine **skips the target**, and the rest of the scan continues with the other targets. After a **15-minute cooldown** the next attempt enters HALF_OPEN — a single probe is allowed; success closes the circuit, failure re-opens it. Per-target overrides (optional YAML key `circuit_breaker:` with `failure_threshold`, `cooldown_seconds`, `retry_attempts`, `retry_max_seconds`, `jitter`) and process-wide envs (`DATA_BOAR_CIRCUIT_FAILURE_THRESHOLD`, `DATA_BOAR_CIRCUIT_COOLDOWN_SECONDS`, `DATA_BOAR_RETRY_ATTEMPTS`, `DATA_BOAR_RETRY_MAX_SECONDS`) are available for incident response. Breaker telemetry (state, failure counts, retry-after seconds, last error class) is exposed under `audit_log.circuit_breakers` for the dashboard / API. Programmer errors (`ValueError`, `TypeError`, `KeyError`) are **never** retried and **never** trip the circuit. See **ADR 0044** and the `DEFENSIVE_SCANNING_MANIFESTO.md` for the contract.
