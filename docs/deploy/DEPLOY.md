...
```bash
# From repo root
docker build -t fabioleitao/data_boar:latest -t fabioleitao/data_boar:1.7.1 .
docker login
...
- For long-term recovery, record the **image tag or digest** you ran (e.g. `fabioleitao/data_boar:1.7.1`) alongside the data backup so you can match behaviour when restoring.
