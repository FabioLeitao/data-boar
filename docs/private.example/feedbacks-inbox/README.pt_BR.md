...
- **Hábito:** coloque aí feedbacks de parceiros ou clientes (drops estilo WRB, análises Corporate-Entity-C, exportações que não devem ir para o `origin`).
...
Quando o operador usar o token de sessão **`feedback-inbox`**, os assistentes fazem **`read_file`** / listam primeiro a pasta gitignored de drops. Se estiver **vazia**, diga isso. Se a **origem** (Corporate-Entity-C vs Gemini vs outra) não estiver clara, **pergunte** antes de atribuir linhas em **`docs/plans/`** ou **`PLANS_TODO.md`**.
