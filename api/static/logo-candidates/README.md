...
This folder holds **logo variants** while we choose the final mark. The selected file will be copied to `api/static/logo.svg` (and PNG/favicon exports) for use in the app.
...
2. Copy the chosen SVG to `api/static/logo.svg`.
3. Export PNG (32 px, 64 px) and favicon (16/32 PNG or ICO) from that SVG.
4. Optionally remove or archive this folder once the final logo is in place.
