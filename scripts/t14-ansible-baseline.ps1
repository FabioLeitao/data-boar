...
if ($Apply -and -not $SkipCheck) {
  $runLines += "ANSIBLE_ROLES_PATH=./roles ansible-playbook -i inventory.local.ini $becomePart playbooks/lab-node-01-baseline.yml --check --diff"
  $runLines += "ANSIBLE_ROLES_PATH=./roles ansible-playbook -i inventory.local.ini $becomePart playbooks/lab-node-01-baseline.yml --diff"
} else {
  $runLines += "ANSIBLE_ROLES_PATH=./roles ansible-playbook -i inventory.local.ini $becomePart playbooks/lab-node-01-baseline.yml $runModeArgs"
}
