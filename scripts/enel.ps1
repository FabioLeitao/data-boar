...
.DESCRIPTION
    ...
    acidentalmente (963288266, 963404846, 963405556) em sessao anterior.
...
.NOTES
    Contato Enel: 0800 722 0120 (para cancelar protocolos acidentais ou verificar status)
...
if ($Show -in @("all","alerts")) {
    ...
    if ($content -match 'FALTA DE LUZ') {
        ...
        Write-Warn "Verificar: 0800 722 0120 ou App Enel"
    } else {
...
if ($Show -in @("all","summary")) {
    ...
    Write-Host "    Ligar 0800 722 0120 com o numero do protocolo" -ForegroundColor White
}
