...
if ($Service -in @("all","linkedin")) {
    ...
    if (Should-Collect "linkedin" $state $interval) {
        ...
        Write-Info "  talent.ps1 linkedin contributor-a"
        Write-Info "  talent.ps1 linkedin colleague-a"
        Write-Info "  talent.ps1 linkedin Colleague-B"
...
if ($Service -in @("all","social")) {
    ...
    if (Should-Collect "social" $state $interval) {
        ...
        foreach ($candidato in @("contributor-a","colleague-a","Colleague-B","dudu")) {
