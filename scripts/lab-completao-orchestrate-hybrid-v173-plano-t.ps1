...
$NodeDef = @(
    @{ Regex = "latitude";  Name = "Latitude-5400"; Type = "engine" }
    @{ Regex = "t14";       Name = "ThinkPad-T14";  Type = "engine" }
    @{ Regex = "mini-bt";   Name = "Beelink-Mini";  Type = "engine" }
    @{ Regex = "pi3b";      Name = "Pi3B-Passive";  Type = "passive" }
)
...
foreach ($n in $Ordered) {
    ...
    try {
        ...
        if ($n.Type -eq "passive") {
            Invoke-Pi3bPassiveSsh -Node $n -RepoPath $n.Repo
            continue
        }
        ...
        $scanPath = if ($n.Name -match "Latitude") { Resolve-LatitudeScanPath -Target $target } else { "/etc" }
        if (-not $scanPath) {
            $scanPath = "/tmp"
        }
