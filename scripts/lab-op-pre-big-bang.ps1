...
.DESCRIPTION
  ...
  3) -IncludeProbes: per-host SSH read-only snippets (ufw/fail2ban where unprivileged; sudo -n best-effort),
     git dirty line count on first repoPath, and on mini-bt / pi3b hosts a compression/stdlib smoke via python3.
...
param(
    ...
    [string] $LabOpCidrsExample = "10.0.0.0/24 192.168.1.0/24"
)
...
[void]$sb.AppendLine("F) Mini-BT Void: sudo xbps-install -S libmariadbclient-devel pkg-config if mysqlclient wheels fail; keep heavy DB work on Latitude/T14.")
[void]$sb.AppendLine("")
...
foreach ($h in $manifest.hosts) {
    ...
    $depBlock = ""
    if ($alias -match "mini-bt|minibt|pi3b") {
