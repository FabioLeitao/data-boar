...
O **`pyproject.toml`** é a fonte de verdade para a toolColleague-Nn **uv**; **pip** e **`requirements.txt`** são derivados (o requirements.txt é gerado a partir do pyproject.toml para ambientes que usam pip ou legados). As dependências são declaradas em **`pyproject.toml`** e gerenciadas principalmente via **uv**:
