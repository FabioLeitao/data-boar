...
- **Dependencies:** The source of truth for libraries is **`pyproject.toml`** (uv toolColleague-Nn); pip and **`requirements.txt`** are derivative. Declare all runtime and dev dependencies in **`pyproject.toml`**. When you add or change deps, run `uv sync` and regenerate the lockfile with `uv pip compile pyproject.toml -o requirements.txt`. Do not edit `requirements.txt` by hand for version changes.
