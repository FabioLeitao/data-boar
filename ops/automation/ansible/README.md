...
- **Docker CE**, **Podman**, and **k3s** are **opt-in** (variables in **`playbooks/lab-node-01-baseline.yml`**). The **`ctop`** binary is installed by default for observability, but **`docker`** is **not** present unless you enable **`lab-node-01_install_docker_ce: true`** (or install **`docker.io`** from Debian separately).
