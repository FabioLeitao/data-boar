...
- **Docker CE** (official repo) **+ Compose plugin** are **on by default** in **`lab-node-01-baseline.yml`** so **`docker`** and **`ctop`** work after one playbook run. Set **`lab-node-01_install_docker_ce: false`** in inventory to skip. **Podman** and **k3s** stay **opt-in**.
