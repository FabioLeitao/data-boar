...
- **Snapper missing / `Unit snapper.service could not be found`:** The **`lab-node-01_snapper`** role only installs Snapper when **root (`/`) is btrfs** (`roles/lab-node-01_snapper/tasks/main.yml`). A default **ext4** install skips the role (by design). Snapper also does not expose a generic `snapper.service` on Debian — use **`snapper-timeline.timer`** / **`snapper-cleanup.timer`** after install. To use Snapper you need a btrfs root (reinstall/partition) or disable expectations with **`lab-node-01_snapper_enabled: false`** in playbook vars.
